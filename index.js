//Erstelle eine Datai mit dem Namen "Giveaway.json" und schreibe einfach nur {} rein. WICHTIG!!! Die Datei muss im selben Ordner wie dieses Script sein :D
...
bot.on("messageCreate", async message=>{
    ...
    if(message.content.startsWith("!g end")){
        if(!g[message.guild.id]) return message.channel.send({content:"Es gibt keine aktiven Giveaways die du beenden könntest. :slight_frown:"});
        let away = message.content.split(" ").slice(2).join("")
        let giveaway;

        if(g[message.guild.id].length < 1) return message.channel.send({content:"Es gibt keine aktiven Giveaways die du beenden könntest. :slight_frown:"})
        ...
        if(!giveaway || giveaway.ended == true) return message.channel.send({content:":x: Das Giveaway wurde beendet oder existiert nicht"})
    ...
    if(message.content.startsWith("!g delete")){
        if(!g[message.guild.id]) return message.channel.send({content:"Es gibt keine aktiven Giveaways die du löschen könntest. :slight_frown:"});
    ...
    if(message.content.startsWith("!g reroll")){
        if(!g[message.guild.id]) return message.channel.send({content:":x: Es gibt keine aktiven Giveaways wo du den Gewinner neu ziehen kannst."});
    
        let away = message.content.split(" ").slice(2).join("")
        let giveaway;

        if(g[message.guild.id].length < 1) return message.channel.send({content:":x: Es gibt keine aktiven Giveaways die du beenden könntest."})

        if(!away){
            giveaway = g[message.guild.id][g[message.guild.id].length-1];

            if(g[message.guild.id][g[message.guild.id].length-1].ended == false) return message.channel.send({content:" :x: Das Giveaway ist noch nicht vorbei."})
            if(g[message.guild.id][g[message.guild.id].length-1].members.length < 1) return message.channel.send({content:":x: Das Giveaway hatte keinen Teilnehmer."})
            ...
            for(i=0;i<g[message.guild.id].length;i++){
                if(g[message.guild.id][i].mId == away){
                    ...
                    if(g[message.guild.id][i].members.length < 2) returnmessage.channel.send({content:"Das Giveaway hatte keinen Teilnehmer."})
        ...
        message.channel.send({content:"Gewinner wird neu gezogen. :hourglass:"})
...
bot.on("interactionCreate", async interaction=>{
    ...
    if(interaction.customId == "giveaway_teilnahme"){
        ...
        if(!gg || gg.ended == true) return interaction.reply({content:"Das Giveaway ist bereits beendet worden.", ephemeral:true})
    ...
    Object.keys(g).forEach(away=>{
        for(i=0;i<g[away].length;i++){
            if(new Date().getTime() > g[away][i].end && g[away][i].ended == false){
                ...
                if(ch){
                    if(g[away][i].members.length > 0){
                        ch.send({content:"Glückwunsch <@!"+winner.join(">, <@!")+">\n"+(winner.length == 1 ? "du hast " : "ihr habt ") + "__**"+g[away][i].price+"**__ gewonnen :tada:"})
                    }
                }
